...
int main()
{
	...
	statusLED = XGpio_Initialize(&leds, LED_DEVICE_ID);
	if (statusLED != XST_SUCCESS) {
		xil_printf("LEDs initialization failed\n\r");
	}

	/* Set data directions */
	XGpio_SetDataDirection(&switches, 1, 0xF0);  /* Upper 4-bits for buttons (input) */
	...
	while (1) 
	{
		...
		buttons = XGpio_DiscreteRead(&swithces, 1);
		...
		if (buttons & 0x80) {  // BTN3 pressed
			...
			xil_printf("Action: Displaying count on LEDs\n\r");
...
int delay(void)
{
	...
	while (delay_count < WAIT_VAL)  // Increase delay count until it reaches WAIT_VAL
		delay_count++;
	return 0;
}
...
/* Definitions */
#define GPIO_DEVICE_ID XPAR_GPIO_0_DEVICE_ID  /* GPIO device for buttons/switches */
#define LED XPAR_LED_DEVICE_ID   /* GPIO device for LEDs */
#define WAIT_VAL 1000000000                   /* Delay value for 1Hz operation */
